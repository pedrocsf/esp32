...
void start_advertising(const char *name)
{
  ...
  adv_data[i++] = 7;    // Length (corrigido para 7 como na band real)
...
void advertising_loop_task(void *param)
{
  while (1)
  {
   ...
   // Simula valores de BPM: 60-100 BPM (valores normais) ou 0 (sem medição)
    // if (current_idx % 3 == 0)
    // {
    //   current_bpm = 0; // Simula "sem medição"
    // }
    // else
    // {
    //   current_bpm = 60 + (current_idx % 40); // BPM entre 60-99
    // }
